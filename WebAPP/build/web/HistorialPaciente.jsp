                ...
                <div class="col-lg-12">
                    <div class="table-responsive">    
                        <%
                            ...
                            if (rs.next()) {
                                rs = smt.executeQuery("select case when mgs.archivo is null then ' ' else 'Download' end archivo1, p.paciente_dni as cedula, (p.paciente_primer_nombre || ' ' || p.paciente_segundo_nombre || ' ' || p.paciente_apellido_paterno || ' ' || p.paciente_apellido_materno) as nombrepaciente, p.paciente_canton as canton, p.paciente_direccion as direccion, TO_CHAR (mgs.fecha:: DATE, 'dd-mm-yyyy') as fecha, mgs.notas as motivoconsulta, mgs.archivo as archivo from mg_seguimiento as mgs inner join medicinageneral as mg on mgs.idmedicinageneral = mg.idmedicinageneral inner join paciente as p on mg.paciente_id = p.paciente_id inner join galeno as g on mg.galeno_id = g.galeno_id where  g.galeno_user = '" + session.getAttribute("galeno_user11") + "' ");
                        %>
